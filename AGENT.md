...
| `nook workspace list\|doctor\|studio` | cross-repo, read-only — see "workspace is read-only" below |
...
**None of the write-authorization rules elsewhere in this file apply here, because there is nothing under `workspace` that writes.** There is no `nook workspace new`, `set`, or `mv` — deciding which member a write belongs to is an open question this tool deliberately leaves unanswered for now. So the `queued` boundary above has nothing to guard against under `workspace`: it exists to gate *content* changes (`new`, `set`, `mv`, `comment`, `label`, and the private/share decision), and `workspace` makes none. `doctor --fix` under `workspace` repairs file-level data integrity the same way `nook doctor --fix` does for one board — that is not the kind of write `queued` is about, and it needs no more asking than the single-board form does.
