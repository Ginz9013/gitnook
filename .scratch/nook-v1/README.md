...
平行執行模式。16 張票，11 個批次。**v1 全數完成並發布**（`gitnook@0.1.0`，2026-09-08）。board 僅存於 `.scratch/`。

## 執行前的前置條件（已解除）

> 規劃當時這個目錄還不是 git repo，而平行 worker 是一票一個 commit，沒有 git repo 就無法派工。
> `git init` 已完成，board 本身也已納入版本控制，需要真實 git 的票（05、06）因此得以執行。
...
| B3 | 03 | 1 | ✅ | OR-Set 與 comments（core 檔案，無法與 04 同批） |
