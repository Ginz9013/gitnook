...
一個 npm 套件。`npm i -D @nook/cli` 之後，`nook` 指令可用；issue 以純文字存在 repo 內、隨 git 走；兩個開發者在各自分支同時修改同一張票，`git merge` **不產生衝突且不遺失任何一方的變更**；`nook studio` 在 localhost 開一個唯讀看板供會議討論。
...
- [ ] `npm i -D @nook/cli` 後 `nook` 指令可用，`nook init` 產生 `.issues/issues/` 與 `.gitattributes` 那一行
