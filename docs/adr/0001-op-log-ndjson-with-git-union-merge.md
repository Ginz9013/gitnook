...
4. **不得真的刪除 Issue 檔。** modify/delete 是 union 不涵蓋的固有衝突。v1 因此完全不提供刪除。**這條規則本身不變，但那句「v1」後來被接上了** —— ADR-0009 讓刪除以 `set deleted=<bool>` 這個 LWW 欄位實作，使用者說得出「這張不要了」，而檔案照樣永不 unlink。真的清掉位元組是 `nook gc` 的事，它會自己承擔這條衝突。
