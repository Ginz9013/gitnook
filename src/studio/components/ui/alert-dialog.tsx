...
/**
 * 確認框。**與 `sheet.tsx`（Radix Dialog）不是同一個東西**，差別不在外觀：
 * AlertDialog 沒有「點外面關掉」，Esc 之外的每一條退路都必須是使用者按了
 * 上面某一顆按鈕，而預設焦點落在 Cancel。那正是「這一下不可逆」該有的行為。
 *
 * focus trap、`role="alertdialog"`、`aria-modal`、scroll lock —— 交給 Radix，
 * 理由同 ADR-0008：手寫這些都會寫錯。
 *
 * 目前唯一的使用者是 drawer 上的刪除（`IssueDetail.tsx`）。這個相依原本宣告在
 * `devDependencies` 裡卻沒有任何一行 import 它 —— 這個檔案是它的第一個使用者。
 */
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        className={cn(
          // z-50 與 drawer 同層：這個框由 drawer 裡的按鈕開出來，必須疊在它上面，
          // 而 Radix 的 Portal 讓它在 DOM 上排在後面 —— 同層時後者勝出。
